...
https://github.com/Jacob-CE/micropython-person-sensor

Modified by Ed Roberts Rayne
"""
...
class PersonSensor(object):
    ...
    def __init__(self, i2c):
        ...
        if int(PERSON_SENSOR_I2C_ADDRESS) not in devices:
            raise Exception(
                f"Could not find Person Sensor at address 0x{PERSON_SENSOR_I2C_ADDRESS:02x}!"
            )
    ...
    def enable_id_model(self, is_enabled):
        ...
        self.i2c.writeto_mem(
            PERSON_SENSOR_I2C_ADDRESS, PERSON_SENSOR_REG_ENABLE_ID, data
        )
    ...
    def single_shot(self):
        self.i2c.writeto_mem(
            PERSON_SENSOR_I2C_ADDRESS, PERSON_SENSOR_REG_SINGLE_SHOT, b"\x00"
        )
    ...
    def label_next_id(self):
        self.i2c.writeto_mem(
            PERSON_SENSOR_I2C_ADDRESS, PERSON_SENSOR_REG_CALIBRATE_ID, b"\x00"
        )
    ...
    def persist_ids(self, is_enabled):
        data = b"\x01" if is_enabled else b"\x00"
        self.i2c.writeto_mem(
            PERSON_SENSOR_I2C_ADDRESS, PERSON_SENSOR_REG_PERSIST_IDS, data
        )
    ...
    def erase_ids(self):
        self.i2c.writeto_mem(
            PERSON_SENSOR_I2C_ADDRESS, PERSON_SENSOR_REG_ERASE_IDS, b"\x00"
        )
    ...
    def enable_debug(self, is_enabled):
        data = b"\x01" if is_enabled else b"\x00"
        self.i2c.writeto_mem(
            PERSON_SENSOR_I2C_ADDRESS, PERSON_SENSOR_REG_DEBUG_MODE, data
        )
